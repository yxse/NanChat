import { ActionSheet, Button, DotLoading, ErrorBlock, List, SafeArea, Space, SwipeAction, Toast } from "antd-mobile";
...
export default function History({ ticker }: { ticker: string }) {
  const { data: history, isLoading } = useSWR("history-" + ticker, () => fetchHistory(ticker));
  const [visible, setVisible] = useState(false);
  const [activeTx, setActiveTx] = useState(null);
  const navigate = useNavigate();
  const actions = [{
    text: "Create contact", key: "add-to-contacts", bold: true
  },
  {
    text: "Copy address", key: "copy-address", onClick: () => {
  ...
  { text: "View Details", key: "view-details", onClick: () => window.open(`https://nanexplorer.com/${networks[ticker].id}/block/${activeTx.hash}`) },
  ]
return (
  <>
            ...
            >
              <SwipeAction
                ...
                rightActions={[{
                  ...
                  color: "#108ee9",
                  onClick: () => {
                    navigate(`/${ticker}/send?to=${tx.account}&amount=${+rawToMega(ticker, tx.amount)}`);
                  },
                  text: <>
                    <BiSolidSend size={18} />
                  </>
