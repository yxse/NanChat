...
import { Button, DotLoading, FloatingBubble } from "antd-mobile";
...
export default function NetworkList({ onClick }) {
        ...
        >
          ...
          <div className="network-balance">
            <div className="text-right">
              <div>
                {balances[ticker].isLoading ? (
                  <DotLoading />
                ) : (
                  balances[ticker].data
                )}
                {!isLoadingPrices && (
                  <div className="text-sm text-gray-400">
                    ~{" "}
                    {+(prices?.[ticker].usd * balances[ticker].data).toFixed(2)}{" "}
                    USD
                  </div>
                )}
              </div>
