...
import { Button, ErrorBlock, Input, Modal, NavBar, ResultPage, Skeleton, Space } from "antd-mobile";
...
export const fetchBalance = async (ticker: string) => {
  ...
  if (balance.error) {
    ...
    let balanceTotal = +rawToMega(ticker, balance.balance) + +rawToMega(ticker, balance.pending);
    return balanceTotal;
  }
}
...
export default function Network({ticker, onBack}: {ticker: string}) {
  ...
  if (selectedAction === "send") {
    return <Send ticker={ticker} onBack={
    ...
    <div className="container  relative mx-auto">
      ...
      >{networks[ticker].name}</NavBar>
        ...
        <div className="mt-1">
          {
            ...
            <Skeleton.Title animated /><span className="ml-2">{ticker}</span>
            </div>
            : balance + " " + ticker
          }
        ...
        <div className="flex justify-center mt-4 space-x-4">
          ...
          <button className="py-2 px-2 rounded-full bg-gray-800 hover:bg-gray-900 text-white" onClick={async () => {
            // window.history.pushState({}, "", "/receive");
            let address = await getAccount(ticker);
            Modal.show({
              closeOnMaskClick: true,
              title: `Receive ${networks[ticker].name}`,
              content: <div className="flex flex-col items-center mb-2">
          ...
          <div className="flex flex-col items-center">
          <button className="py-2 px-2 rounded-full bg-gray-800 hover:bg-gray-900 text-white" onClick={() => {
            // window.history.pushState({}, "", "/receive");
